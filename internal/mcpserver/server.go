...
func New(api webmaster.Webmaster, readOnlyMode ...bool) *mcp.Server {
	...
	mcp.AddTool(server, &mcp.Tool{Name: "get_summary", OutputSchema: objectSchema(), Annotations: readOnly, Description: "Return site statistics including SQI, searchable pages and problem counts."}, observe("get_summary", tools.GetSummary))
	...
	mcp.AddTool(server, &mcp.Tool{Name: "get_diagnostics", OutputSchema: objectSchema(), Annotations: readOnly, Description: "Return site diagnostics grouped by severity."}, observe("get_diagnostics", tools.GetDiagnostics))
	mcp.AddTool(server, &mcp.Tool{Name: "get_popular_queries", OutputSchema: objectSchema(), Annotations: readOnly, Description: "Return popular search queries with shows, clicks and positions."}, observe("get_popular_queries", tools.GetPopularQueries))
	mcp.AddTool(server, &mcp.Tool{Name: "get_query_history", OutputSchema: objectSchema(), Annotations: readOnly, Description: "Return aggregated search-query statistics over time."}, observe("get_query_history", tools.GetQueryHistory))
